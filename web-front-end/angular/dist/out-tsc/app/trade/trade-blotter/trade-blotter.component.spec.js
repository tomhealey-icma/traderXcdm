...
describe('TradeBlotterComponent', () => {
    ...
    it('should call getTrades on changes and set trades', fakeAsync(() => {
        ...
        component.ngOnChanges({ account: { currentValue: dummyAccounts[0] } });
    ...
    it('should call getTrades and subscribe to trade feed service for given account', async () => {
        spyOn(component.tradeService, 'getTrades').and.callThrough();
        spyOn(component.tradeFeed, 'subscribe').and.callThrough();
        const testAccount = dummyAccounts[0];
        component.ngOnChanges({ account: { currentValue: testAccount } });
        expect(component.tradeService.getTrades).toHaveBeenCalledWith(testAccount.id);
        expect(component.tradeFeed.subscribe).toHaveBeenCalled();
    });
    it('getRowNodeId should return id from trade data', () => {
        expect(component.getRowNodeId(trades[0])).toEqual(trades[0].id);
    });
});
//# sourceMappingURL=trade-blotter.component.spec.js.map