...
describe('Account tests', () => {
    ...
    it('should fetch accounts list on init', async () => {
        spyOn(comp.accountService, 'getAccounts').and.callThrough();
        expect(comp.accountService.getAccounts).not.toHaveBeenCalled();
        comp.ngOnInit();
        await sleep(250);
        fixture.detectChanges();
        expect(comp.accountService.getAccounts).toHaveBeenCalled();
    });
    it('should select the account from account list', async () => {
        ...
        const rows = Array.from(element.querySelectorAll('#accountgrid .ag-row'));
        rows[0].click();
        await sleep(100);
    ...
    xit('should fetch account users for select account', async () => {
        spyOn(comp.accountService, 'getAccountUsers').and.callThrough();
        comp.ngOnInit();
        fixture.detectChanges();
        await sleep(250);
        ...
        const rows = Array.from(element.querySelectorAll('#accountgrid .ag-center-cols-container .ag-row'));
        console.log(rows[0]);
        rows[0].click();
        fixture.detectChanges();
        await fixture.whenRenderingDone();
        expect(comp.accountService.getAccountUsers).toHaveBeenCalled();
