$dir = '.\'
$folder = '..\people-service\PeopleService.WebApi'
$subdir = "$dir$folder"
$outputfile = "\log.txt"
...
$process = Start-Process powershell.exe ("-NoExit " + "cd $dir$folder;dotnet run") 

#$process = Start-Process powershell.exe ("-noexit " + "java --version")

echo 'Starting Trade Feed Server - DB logs below'
echo $process
echo '---------------------------------------------------------------------------'
