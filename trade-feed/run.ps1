...
$process = Start-Process powershell.exe ("-NoExit " + "cd $dir$folder;npm ci;npm run start") 

#$process = Start-Process powershell.exe ("-noexit " + "java --version")

echo 'Starting Trade Feed Server - DB logs below' | Out-File -filepath $dir$folder$statusfile
echo $process
echo '---------------------------------------------------------------------------'
