...
$process = Start-Process powershell.exe ("-NoExit " + "cd $dir$folder;npm run start:prod | Out-File -filepath $dir$folder$outputfile") 

#$process = Start-Process powershell.exe ("-noexit " + "java --version")

echo 'Starting Ref Data Server - DB logs below' | Out-File -filepath $dir$folder$statusfile
echo $process
echo '---------------------------------------------------------------------------'
